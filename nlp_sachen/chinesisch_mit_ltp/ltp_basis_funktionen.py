...
def main() -> None:
    ...
    output = ltp.pipeline(["他写的书很有意思。我觉得他今天不会来滑冰，因为他不舒服。"], tasks=["cws", "pos", "ner", "srl", "dep", "sdp", "sdpg"])
    # 使用字典格式作为返回结果
    # print(f"{list(output.keys())}")
    for task_name, result in output.items():
        print(f"{task_name} : {result}")   # print(output[0]) / print(output['cws']) # 也可以使用下标访问
